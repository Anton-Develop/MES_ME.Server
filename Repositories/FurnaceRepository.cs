...
public interface IFurnaceRepository
{
    ...
    Task<IEnumerable<HeatingSession>> GetSessionsBySheetKeyAsync(int sheet, int? melt, int? partNo, int? pack, CancellationToken ct = default);
...
public sealed class FurnaceRepository : IFurnaceRepository
{
    ...
    public async Task<IEnumerable<HeatingSession>> GetSessionsBySheetKeyAsync(int sheet, int? melt, int? partNo, int? pack, CancellationToken ct = default)
    {
        return await _retry.ExecuteAsync(async () =>
        {
            await using var con = await OpenAsync(ct);
            return await con.QueryAsync<HeatingSession>(Sql.SessionsBySheetKey, new
            {
                Sheet = sheet,
                Melt = melt,
                PartNo = partNo,

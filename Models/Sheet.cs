...
{
    [Table("inputdata", Schema = "mes")]  // Указывает, что модель соответствует таблице InputData
    public class Sheet
    {
        [Key]
        [Column("id")] // ← Явно указываем имя столбца в БД
        public int Id { get; set; }

        [Column("status")]
        public string Status { get; set; } = "New"; // Статус листа (пункт 1.d) - значение по умолчанию

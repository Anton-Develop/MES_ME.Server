...
namespace MES_ME.Server.Controllers
{
    ...
    public class OpcUaController : ControllerBase
    {
        ...
        // GET /api/opc/values/controller/PLC210
        [HttpGet("values/controller/{controllerName}")]
        public IActionResult GetByController(string controllerName)
        {
            var values = ((OpcUaService)_opc).GetControllerValues(controllerName);
            var connected = _opc.IsControllerConnected(controllerName);
            return Ok(new { controller = controllerName, connected, values });
        }

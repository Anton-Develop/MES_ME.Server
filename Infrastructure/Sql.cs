...
internal static class Sql
{
    ...
    WITH
      ...
      with_flag AS (
          SELECT *,
              CASE WHEN gap IS NULL OR gap > INTERVAL '3 minutes' THEN 1 ELSE 0 END AS is_new_session
          FROM presence
      ),
      with_session AS (
          SELECT *,
    ...
    AND agg.exited_at < NOW() - (2 || ' minutes')::INTERVAL
    AND agg.total_minutes <= 50
    AND agg.entered_at_f1 IS NOT NULL
    AND (
          agg.entered_at_f2 IS NOT NULL
          OR agg.entered_at_f3 IS NOT NULL
          OR agg.entered_at_f4 IS NOT NULL
        )    			),
      		existing_max AS (
